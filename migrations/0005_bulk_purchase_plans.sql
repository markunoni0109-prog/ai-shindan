...
-- D1-safe rebuild for 1/10/30/50 prediction plans + My Predictions access token.
-- IMPORTANT: Do not use PRAGMA foreign_keys=OFF. Cloudflare D1 migrations run with FK enforcement.
--
-- Strategy:
-- 1) Rename the whole FK-connected graph first, preserving the old graph.
-- 2) Drop old named indexes/triggers that would conflict with recreated names.
-- 3) Create the new graph and copy data while old referenced rows still exist.
-- 4) Drop the old graph child -> parent.

-- Rename dependent children first.
...
-- Rename parent graph child -> parent.
...
-- Names belong to schema globally; release the names before recreating them.
...
CREATE UNIQUE INDEX idx_purchase_intents_session_id
ON purchase_intents(stripe_checkout_session_id);
...
(id,prediction_id,display_sequence,draw_number,
 ...
 combination_key,generated_at,payment_id,entitlement_id,prediction_index,
 plan_code,algorithm_version,previous_hash,record_hash,created_at)
SELECT
 id,prediction_id,display_sequence,draw_number,
 number_1,number_2,number_3,number_4,number_5,number_6,
 combination_key,generated_at,payment_id,entitlement_id,prediction_index,
...
BEGIN SELECT RAISE(ABORT,'predictions are immutable: UPDATE is forbidden'); END;

CREATE TRIGGER trg_predictions_no_delete
BEFORE DELETE ON predictions
BEGIN SELECT RAISE(ABORT,'predictions are immutable: DELETE is forbidden'); END;
...
CREATE INDEX idx_prediction_matches_prediction
ON prediction_matches(prediction_id,tracking_type);
CREATE INDEX idx_prediction_matches_draw ON prediction_matches(draw_id);

CREATE TRIGGER trg_prediction_matches_no_update
BEFORE UPDATE ON prediction_matches
BEGIN SELECT RAISE(ABORT,'prediction_matches are immutable'); END;

CREATE TRIGGER trg_prediction_matches_no_delete
BEFORE DELETE ON prediction_matches
BEGIN SELECT RAISE(ABORT,'prediction_matches are immutable'); END;

-- ---------- prediction_tracking_summary ----------
...
INSERT INTO prediction_tracking_summary
SELECT * FROM prediction_tracking_summary_old_0005;

-- ---------- prediction_generation_features ----------
CREATE TABLE prediction_generation_features (
  prediction_id TEXT PRIMARY KEY REFERENCES predictions(prediction_id),
  features_json TEXT NOT NULL,
  captured_at TEXT NOT NULL
);
INSERT INTO prediction_generation_features
SELECT * FROM prediction_generation_features_old_0005;

-- Drop old graph in strict child -> parent order.
