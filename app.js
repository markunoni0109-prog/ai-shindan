(function () {
  ...
  async function loadMaster() {
    // Data files live under /data/ (see repo structure). Falls back to the repo
    // root for compatibility with earlier deployments that don't have that folder yet.
    const candidates = ["data/data_master.json", "data_master.json"];
    let loaded = null;
    for (const path of candidates) {
      try {
        ...
        if (res.ok) {
          ...
          state.dataBasePath = path.includes("/") ? "data/" : "";
          break;
        }
      } catch (e) {
        /* try next candidate */
  ...
  function applyLangToStaticUI() {
    const T = t();
    if (el.emergencyLabel) el.emergencyLabel.textContent = T.emergencyLabel;
    if (el.emergencyHint) el.emergencyHint.textContent = T.emergencyHint;
    if (el.searchInput) el.searchInput.placeholder = T.searchPlaceholder;
    if (el.filter24h) el.filter24h.textContent = "🕐 " + T.filter24h;
    if (el.filterWheelchair) el.filterWheelchair.textContent = "♿ " + T.filterWheelchair;
    if (el.filterBaby) el.filterBaby.textContent = "👶 " + T.filterBaby;
    if (el.viewBrowse) el.viewBrowse.textContent = T.viewBrowse;
    if (el.viewRanking) el.viewRanking.textContent = T.viewRanking;
    if (el.footerNote) el.footerNote.textContent = T.footerNote;
    if (el.installText) el.installText.textContent = T.installText;
    if (el.installBtn) el.installBtn.textContent = T.installBtn;
    if (el.offlineBanner) el.offlineBanner.textContent = T.offlineBanner;
    if (el.langToggle) el.langToggle.textContent = state.lang === "ja" ? "EN" : "日本語";
  ...
  function switchView(view) {
    ...
    if (el.filterControls) el.filterControls.style.display = view === "ranking" ? "none" : "";
    renderMain();
  }

  function bindEvents() {
    if (el.emergencyBtn) el.emergencyBtn.addEventListener("click", triggerSOS);
